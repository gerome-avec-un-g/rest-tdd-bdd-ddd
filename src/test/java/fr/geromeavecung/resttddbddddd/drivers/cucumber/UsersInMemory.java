...
public class UsersInMemory implements Users {
    ...
    private final List<UserJson> values;

    public UsersInMemory() throws JsonProcessingException {
        this.values= new ObjectMapper().readValue(JSON, new ObjectMapper().getTypeFactory().constructCollectionType(List.class, UserJson.class));
    }

    @Override
    public User retrieveUser(UserIdentifier userIdentifier) {
        return values.stream()
                .filter(userJson -> userJson.id() == userIdentifier.value())
                .map(UserJson::toDomain)
