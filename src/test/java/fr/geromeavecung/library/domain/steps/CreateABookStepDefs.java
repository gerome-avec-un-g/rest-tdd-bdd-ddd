...
import fr.geromeavecung.library.domain.fakes.BooksInMemory;
...
public class CreateABookStepDefs {
    ...
    public Book book(Map<String, String> row) {
        ...
        return new Book(new ISBN(row.get("ISBN")), new BookTitle(row.get("title")), PublicationDate.read(Integer.parseInt(row.get("publication date"))), UUID.fromString(row.get("author identifier")));
    }
    ...
    public void theBookWithItsUniqueIdentifierAedEbFFbEFffFForAuthorBbDafCCFABeAeAAc(String title, String publicationDate, String isbn, String authorIdentifier) {
        booksInMemory.save(new Book(new ISBN(isbn), new BookTitle(title), PublicationDate.read(Integer.parseInt(publicationDate)), UUID.fromString(authorIdentifier)));
    }
    ...
    public void i_create_a_book_titled_for_author(String title, String publicationDate, String isbn, String authorIdentifier) {
        try {
            book = createABook.execute(new CreateABookCommand(SharedStepDefs.sanitize(isbn), SharedStepDefs.sanitize(title), SharedStepDefs.sanitize(publicationDate), authorIdentifier));
    ...
    public void the_book_is_created_with_its_unique_identifier_for_author(String title, String publicationDate, String isbn, String authorIdentifier) {
        ...
        assertThat(book).isEqualTo(new Book(new ISBN(isbn), new BookTitle(title), PublicationDate.read(Integer.parseInt(publicationDate)), UUID.fromString(authorIdentifier)));
    }
